"""Summarize current-sweep validation runs and extract matched exit tracks."""
...
CURRENT_PATTERN = re.compile(r"CS_I(?P<current>\d+(?:p\d+)?)nA", re.IGNORECASE)
EVENT_COLUMNS = [
    "track_id",
    "status_code",
    "status",
...
def _label_current_na(name: str) -> float:
    match = CURRENT_PATTERN.search(name)
    if match is None:
        raise ValueError(f"Cannot parse source current from case name: {name}")
    return float(match.group("current").replace("p", "."))
...
def _read_case(
    case_dir: Path,
    stable_start_ms: float,
    stability_bin_ms: float,
    ...
    diagnostics = result["terminal_event_diagnostics"]
    ...
    exit_weights = exits["represented_real_ions"].to_numpy(dtype=np.float64)
    exit_radius_mm = 1.0e3 * exits["r_m"].to_numpy(dtype=np.float64)
    ...
    row: dict[str, object] = {
        ...
        "stable_exit_beam_radius_mm_weighted_median": weighted_quantile(
            exit_radius_mm,
            exit_weights,
            0.50,
        ),
        "stable_exit_beam_radius_mm_weighted_p95": weighted_quantile(
            exit_radius_mm,
            exit_weights,
            0.95,
        ),
...
def _trajectory_payloads(case_payloads: list[dict[str, object]]) -> list[dict[str, object]]:
    ordered = sorted(case_payloads, key=lambda payload: float(payload["row"]["source_current_nA"]))
    if len(ordered) <= 3:
        return ordered
    currents = np.asarray([float(payload["row"]["source_current_nA"]) for payload in ordered])
    selected_indices = {0, len(ordered) - 1}
    baseline = int(np.argmin(np.abs(currents - 2.0)))
    selected_indices.add(baseline)
...
def _select_matched_tracks(
    ...
    if len(common_ids) < count:
        raise ValueError(
            f"Only {len(common_ids)} track IDs exit in all selected current cases; {count} requested."
        )
    energy_maps = [
...
def _extract_tracks(
    ...
    for payload in payloads:
        ...
        current_na = float(payload["row"]["source_current_nA"])
        ...
        for label, track_id in selected:
            ...
            track.insert(0, "source_current_nA", current_na)
    ...
    return pd.concat(rows, ignore_index=True).sort_values(
        ["representative_group", "track_id", "source_current_nA", "time_s"]
    )
...
def _plot_tracks(tracks: pd.DataFrame, output: Path, dpi: int) -> None:
    if tracks.empty:
        return
    currents = sorted(tracks["source_current_nA"].unique())
    palette = dict(zip(currents, ("#0072B2", "#009E73", "#D55E00")))
    line_styles = {
        "lower-exit-KE": "-",
        "median-exit-KE": "--",
        "upper-exit-KE": ":",
    ...
    for (current_na, label, track_id), subset in tracks.groupby(
        ["source_current_nA", "representative_group", "track_id"], sort=False
    ):
        subset = subset.sort_values("time_s")
        line_label = f"{current_na:g} nA, {label}, track {int(track_id)}"
        style = line_styles.get(str(label), "-")
        color = palette[float(current_na)]
        axes[0].plot(subset["z_mm"], subset["r_mm"], style, color=color, lw=1.25, label=line_label)
    ...
    axes[0].legend(frameon=False, fontsize=6.6, ncol=3)
...
def main() -> None:
    ...
    case_dirs = sorted(
        (
            ...
            if path.is_dir()
            and CURRENT_PATTERN.search(path.name)
            and (path / "terminal_events.csv").exists()
        ),
        key=lambda path: _label_current_na(path.name),
    )
    if not case_dirs:
        raise FileNotFoundError(f"No current-sweep cases found under {root}")
    ...
    summary = pd.DataFrame(summary_rows).sort_values("source_current_nA")
    trajectory_payloads = _trajectory_payloads(payloads)
    ...
    summary_output = (args.output_summary_csv or root / "02_current_sweep_stable_summary.csv").resolve()
    trajectory_output = (
        args.output_trajectories_csv
        or root / "02_current_sweep_representative_exit_trajectories.csv"
    ).resolve()
    plot_output = (
        args.output_trajectories_plot
        or root / "02_current_sweep_representative_exit_trajectories.png"
    ).resolve()
    ...
    selected_currents = [float(payload["row"]["source_current_nA"]) for payload in trajectory_payloads]
    print(f"cases={len(summary)}")
    print(f"trajectory_currents_nA={selected_currents}")
